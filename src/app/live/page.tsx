...
function LiveContent() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background">
        <div className="text-center space-y-4">
          ...
          <p className="font-black text-xs text-muted-foreground uppercase tracking-widest">Connecting to Live Feed...</p>
        </div>
      </div>
    );
  }
  ...
  if (!match) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-background p-6 text-center">
        ...
        <h2 className="text-2xl font-black text-foreground uppercase tracking-tight">Match Offline</h2>
        <p className="text-xs text-muted-foreground font-bold mt-2 uppercase">Scoreboard unavailable or invalid ID.</p>
  ...
  return (
    <div className="min-h-screen bg-background pb-24 font-body">
      <header className="sticky top-0 z-50 bg-background/80 backdrop-blur-md border-b px-4 py-3 flex justify-between items-center shadow-sm">
        <div className="flex items-center gap-3 overflow-hidden">
          ...
          <div className="truncate">
            <h2 className="font-black text-[10px] text-muted-foreground uppercase leading-none mb-1">SPECTATING</h2>
            <h1 className="font-bold text-sm leading-none truncate text-foreground">{battingTeam.name}</h1>
          </div>
        </div>
        <div className="flex flex-col items-end gap-1">
          <div className="flex items-center gap-2">
            {isRefreshing && <RefreshCcw className="w-3 h-3 text-primary animate-spin" />}
            <Badge variant="outline" className={`text-[9px] font-black uppercase ${match.status === 'ongoing' ? 'text-destructive border-destructive/20' : 'text-muted-foreground border-border'}`}>
              {match.status === 'ongoing' ? '🔴 LIVE' : 'FINISHED'}
            </Badge>
          </div>
          <span className="text-[8px] font-bold text-muted-foreground/40 uppercase tracking-tighter">Syncs every 3s</span>
        </div>
      </header>
            ...
            <div>
               ...
               <div className="flex gap-2 flex-wrap">
                {lastBalls.length > 0 ? lastBalls.map((ball, i) => (
                  ...
                  <span className="text-xs text-muted-foreground/40 italic font-medium">Waiting for next ball...</span>
                )}
               </div>
            </div>
          </CardContent>
                    ...
                    {battingTeam.players.map((p) => (
                      <TableRow key={p.id} className="border-border/50 hover:bg-muted/20">
                        <TableCell className="py-3">
                          <p className={`font-bold text-xs ${p.id === match.currentStrikerId ? 'text-primary' : 'text-foreground'}`}>
                ...
                <Table>
                  ...
                  <TableBody>
                    {bowlingTeam.players.filter(p => p.oversBowled > 0 || p.id === match.currentBowlerId).map((p) => (
                      <TableRow key={p.id} className="border-border/50 hover:bg-muted/20">
                        <TableCell className={`py-3 font-bold text-xs ${p.id === match.currentBowlerId ? 'text-secondary' : 'text-foreground'}`}>
      ...
      <footer className="fixed bottom-0 inset-x-0 bg-background/80 backdrop-blur-md border-t px-4 py-2 flex justify-around items-center safe-paddings shadow-lg z-50">
        <Button 
          variant="ghost" 
        ...
        <DialogContent className="rounded-3xl max-w-[90vw] p-6">
          ...
          <div className="space-y-6 py-4">
            <div className="space-y-2">
              <label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Match UID</label>
              <div className="flex gap-2">
                <div className="flex-1 bg-muted/50 border border-border/50 rounded-xl px-4 py-3 font-mono text-xs break-all select-all text-foreground">
                  {match.id}
                </div>
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Spectator Link</label>
              <div className="flex gap-2">
                <div className="flex-1 bg-muted/50 border border-border/50 rounded-xl px-4 py-3 font-mono text-[10px] break-all select-all text-primary">
...
export default function LivePage() {
  return (
    <Suspense fallback={<div className="p-8 text-center font-black uppercase tracking-widest text-[10px] text-muted-foreground">Loading...</div>}>
      <LiveContent />
    </Suspense>
  );
}