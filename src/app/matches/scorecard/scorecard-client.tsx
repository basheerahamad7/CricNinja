...
import { ArrowLeft, Trophy, Printer, PlusCircle, Home, Star, User, UserPlus } from 'lucide-react';
...
function ScorecardContent() {
  ...
  const syncToFirestore = (updatedMatch: Match) => {
    if (db && user && updatedMatch.ownerId === user.uid) {
  ...
  const handleAddPlayer = (teamId: string) => {
    ...
    const newPlayer: Player = {
      id: uuidv4(),
      name: `Player ${targetTeam.players.length + 1}`,
      runs: 0,
      balls: 0,
      fours: 0,
      sixes: 0,
  ...
  if (!match) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center">
        ...
        <h2 className="text-xl font-black">Scorecard Unavailable</h2>
    ...
    <div className="min-h-screen bg-gray-50 pb-24 font-body">
      ...
      <Dialog open={isRenameDialogOpen} onOpenChange={setIsRenameDialogOpen}>
        <DialogContent className="rounded-3xl max-w-[90vw]">
          <DialogHeader>
            <DialogTitle className="text-center font-black uppercase">Rename Player</DialogTitle>
          </DialogHeader>
          ...
          <DialogFooter>
            <Button className="w-full h-12 rounded-2xl font-black uppercase" onClick={handleRenamePlayer}>
              Update Name
            </Button>
          </DialogFooter>
        </DialogContent>
